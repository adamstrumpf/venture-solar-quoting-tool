# venture-solar-quoting-tool — AI Assistant Instructions
...
## Project context:
- **Description**: A standardized quoting tool for Venture Home Solar's post-install service work, replacing inconsistent manual pricing with automated calculations based on labor rates, materials, and state requirements.
...
## Quick commands (run from repo root: `~/Documents/Claude/projects/venture-solar-quoting-tool`):
```bash
npm run dev                                       # local dev server
docker build -t venture-solar-quoting-tool .                        # test Docker build locally
